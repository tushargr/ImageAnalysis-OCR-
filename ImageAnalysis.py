#non text filter,attached characters in binary image,seperate merged word, mser not detecting i-can increase contrast before mser,a appearing 3
import scipy
...
def marktext(newimg,top_left,bottom_right):
...
regions = mser.detect(grayimg)                  #height,width
...
while(i<len(regions)):
    ...
    top_left=np.min(regions[i], axis=0)
    bottom_right = np.max(regions[i], axis=0)
    ...
    if((ratio<0.1) or (ratio>10)):
	    regions.pop(i)
	    continue
    if((1.00*dy/grayimg.shape[0]>0.8) or (1.00*dy/grayimg.shape[0]<0.01)):
	    regions.pop(i)
	    continue
    if((1.00*dx/grayimg.shape[1]>0.8) or (1.00*dx/grayimg.shape[1]<0.001)):
	    regions.pop(i)
	    continue
    #cv2.imwrite('%d.png'%(i),extract_region(img, regions[i]))
    newimg=marktext(newimg,top_left,bottom_right)
    i+=1
...
for i in range(0,len(words)):
    ...
    contours,hierarchy = cv2.findContours(thresh3, 1, 2)
